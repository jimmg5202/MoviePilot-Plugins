...
class ChineseSubFinder(_PluginBase):
 ...
 # 替换文件扩展名为.mp4
    for file_path in item_file_list:
        ...
        # 路径替换
        if self._local_path and self._remote_path and new_file_path.startswith(self._local_path):
            new_file_path = new_file_path.replace(self._local_path, self._remote_path).replace('\\', '/')
